...
public class CarInventory {
    ...
    public CarInventory() {
        ...
        this.carInventory = hf.JSONReader();
        if(carInventory.size() == 0) {
            carID = 0;
        } else {
            carID = carInventory.get(carInventory.size() - 1).getIDNumber() + 1;
        }
    }
    ...
    static void setCarID(int setCarIDIn) {
        carID = setCarIDIn;
    }
    ...
    public void addCar() {
        ...
        Car carTemp = new Car(make, model, year, color, miles, carID);
        carID++;

        // Add the car to the carInventory
        carInventory.add(carTemp);

        // Write the car to the carFile.json
        hf.JSONWriter(carInventory);
